...
class JwtDecoderService():
    ...
    def _decode_header_thread(self, task, source_object, task_data, cancelable):
        if task.return_error_if_cancelled():
            return
        outcome = self._decode_header(self._token)
        task.return_value(outcome)

    def _decode_payload_thread(self, task, source_object, task_data, cancelable):
        if task.return_error_if_cancelled():
            return
    ...
    def decode_finish(self, result, caller: GObject.Object):
