    ...
    def _convert(self):
        ...
        if self.compress_direction:
            ...
            if self.input_is_text:
                ...
                if len(input_text) > 0:
                    compressed = gzip.compress(input_text.encode())
                    output_buffer.set_text(base64.b64encode(compressed).decode("utf-8"))
            else:
                compressed = gzip.compress(self.image_bytes)
                output_buffer.set_text(base64.b64encode(compressed).decode("utf-8"))
        else:
            if self.input_is_text:
                input_buffer = self.input_textview.get_buffer()
                ...
                output_buffer = self.output_textview.get_buffer()
                try: 
                    gziped = base64.b64decode(input_text)
                    decoded_data = gzip.decompress(gziped).decode("utf-8")
                    output_buffer.set_text(decoded_data)
                    self.output_type_stack.set_visible_child_name("text")
                except UnicodeDecodeError:
                    gziped = base64.b64decode(input_text)
                    decoded_data = gzip.decompress(gziped)
                    texture = Gdk.Texture.new_from_bytes(GLib.Bytes(decoded_data))
                ...
                self.toast.add_toast(Adw.Toast(title=_("Cannot decompress from an image")))
